...
def add_backbone_pos_res(system, positions, atoms, restraint_force):
  force = CustomExternalForce("k_res*paeriodicdistance(x, y, z, x0, y0, z0)^2")
...
pos_res_sys = add_backbone_pos_res(system, modeller.positions, modeller.topology.atoms(), 100)
...
simulation = Simulation(modeller.topology, pos_res_sys, integrator)
...
# Append reporters
simulation.reporters.append(DCDReporter(('traj_01.dcd'), 1000))
...
simulation.reporters.append(PDBReporter(('prod_01_snapshots.pdb'), reportInterval = 10000))
